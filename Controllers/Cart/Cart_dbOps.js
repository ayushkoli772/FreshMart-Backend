...
var newCartitem=async (req,res)=>{
  ...
  console.log('AAAAAAAAAAAAAAAAAAAAAAA searching cartId for userId:'+userId)
...
var cartitemId =0;
 ...
 if(boolExistsProductInCart==false){
  ...
  console.log('trying to inserting cartitem with quantity:'+item.quantity)
        await transaction.request()
         .input('cartId', sql.Int, cartId)
          ...
          .input('pmultq',sql.Decimal,item.pmultq)        
       ...
       .input('CREATEDBY_username',sql.VarChar,item.CREATEDBY_username)
          .query(`INSERT INTO cartItems (cartId,seller_id,seller_name, productId,product_name,price, quantity,pmultq,CPorCQ,selectedUnit,CREATEDBY_username) 
                              VALUES (@cartId,@seller_id,@seller_name, @productId,@product_name,@price, @quantity,@pmultq,@CPorCQ,@selectedUnit,@CREATEDBY_username)`);
      }
else{console.log('already existsing item in cartitem');console.log('now quantity is:'+item.quantity);console.log('now pmulq is:'+item.pmultq)
await transaction.request()
         .input('cartId', sql.Int, cartId)
       ...
       .input('pmultq',item.pmultq)
       .input('CPorCQ',item.CPorCQ)
       .input('selectedUnit',item.selectedUnit)
       
       .input('CREATEDBY_username',item.CREATEDBY_username)

          .query(`Update cartItems set quantity=@quantity, price=@price,pmultq=@pmultq, CPorCQ=@CPorCQ, selectedUnit=@selectedUnit where cartId=@cartId and productId=@productId`);
...
getCart=async (req,res)=>{ 
 ...
 console.log('getCart id:'+req.params.id)
  let pool = await new sql.ConnectionPool(sqlConfigFile).connect();
  const request = pool.request();
                         
                 var sp_name='sp_Cart_Get';    
            try{
 ...
 module.exports={getCart,newCartitem,deleteCartitem}